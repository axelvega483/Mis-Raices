...
public class AdapterPedidoDetalle extends RecyclerView.Adapter<AdapterPedidoDetalle.ViewHolder> {
    ...
    public AdapterPedidoDetalle(List<PedidoDetalle> pedidoDetalles, Context context, PedidoViewModel pedidoViewModel) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.binding.btnSumar.setOnClickListener(v -> {
            ...
            if (cantidadActual < stockDisponible) {
                pedidoDetalle.setCantidad(cantidadActual + 1);
                holder.binding.txtCantidad.setText(String.valueOf(pedidoDetalle.getCantidad()));

                notifyItemChanged(position);
                pedidoViewModel.actualizarListaPedidos(pedidoDetalles);
            } else {
                Toast.makeText(context, "Excede la cantidad de stock disponible", Toast.LENGTH_SHORT).show();
            }
        });
